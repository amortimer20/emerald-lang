...
Numeric `.format()`/`to_string(base:)` (15.5's sketch) is next: additions to `Int`/`Float`'s
method tables, no core interpreter changes needed. The `Textual` trait for custom
print/interpolation display comes after that, but deserves its own design pass first — it
changes core display behavior for every struct and class, including open questions (does a
nested object inside a `List`/`Dict` display through it, or only a top-level value?) that
shouldn't be decided inside an implementation diff. Otherwise, the LSP's second phase (hover,
...
The latest completed slices, including `remove_if`, passed `bash tools/check-toolchain.sh`,
...
commit `f671b1f` (`Warn about impossible class type tests`) before this pass.
