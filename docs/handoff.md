...
| 7 | Diagnostics across all six slices, including items noted in earlier chunks: a set of an ineligible type reported twice, key and set help that does not mention enums, and "Inside `if a is Dog { ... }`" help given inside that very `if` after an assignment undid the narrowing, an operator's help for an optional right operand that does not mention checking for `nothing`, a second operator error cascading from a failed `with`, and the `self.cents - other.cents` comparison idiom in `examples/operators.em` that overflows at the ends of `Int` | Pending |
