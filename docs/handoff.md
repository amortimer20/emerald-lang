...
| 7 | Diagnostics across all six slices, including items noted in earlier chunks: a set of an ineligible type reported twice, key and set help that does not mention enums, and "Inside `if a is Dog { ... }`" help given inside that very `if` after an assignment undid the narrowing | Pending |
