...
from other engines listed in [`docs/library/regex.md`](library/regex.md). The design record
is [`regex-design-plan.md`](regex-design-plan.md); `python3 tools/regex-differential.py
...
Regex slice 6, the last, is committed; slice 4 also fixed the formatter, which printed `?.` as `.`. With pinned Zig 0.16.0, Debug and ReleaseSafe `zig build test`
